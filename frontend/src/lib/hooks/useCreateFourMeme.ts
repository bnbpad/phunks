    ...
    async (
      ...
      try {
        ...
        if (!fourMemeApi.isApiSuccess(nonceResponse)) {
          throw new Error(
            (nonceResponse as any)?.msg || "Failed to generate nonce"
          );
        }
        ...
        if (!fourMemeApi.isApiSuccess(loginResponse)) {
          throw new Error(
            (loginResponse as any)?.msg || "Authentication failed"
          );
        }
        ...
        if (!imageUrl || imageUrl.length <= 0) {
          ...
          if (!fourMemeApi.isApiSuccess(uploadResponse)) {
            throw new Error(
              (uploadResponse as any)?.msg || "Failed to upload image"
            );
          }
        ...
        if (/User rejected|rejected by user|user denied/i.test(errorMessage)) {
          ...
          const fullErrorMessage =
            "Creation of token failed: " + errorMessage.slice(0, 120);
