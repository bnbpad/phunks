...
contract AgentNFT is ERC721EnumerableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function createAgent(
        ...
        agentStates[tokenId] =
            AgentState({balance: 0, active: true, logicAddress: logicAddress, createdAt: block.timestamp});
    ...
    function getAgentState(uint256 tokenId)
        external
        view
        returns (uint256 balance, bool active, address logicAddress, uint256 createdAt, address owner)
    {
        require(_exists(tokenId), "Token does not exist");
    ...
    function getAgentMetadata(uint256 tokenId)
        external
        view
        returns (AgentMetadata memory metadata, string memory metadataURI)
    {
        require(_exists(tokenId), "Token does not exist");
