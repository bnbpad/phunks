import { isEIP55Address, ParsedMessage, parseIntegerNumber } from '@spruceid/siwe-parser';
...
export class SiweMessage {
  ...
  constructor(param: string | Partial<SiweMessage>) {
    if (typeof param === 'string') {
      ...
      this.statement = param?.statement || null;
      this.uri = param.uri || '';
      this.version = param.version || '';
      this.chainId = param.chainId || 0;
      this.nonce = param.nonce || '';
      this.issuedAt = param?.issuedAt;
      this.expirationTime = param?.expirationTime || null;
      this.notBefore = param?.notBefore || null;
      this.requestId = param?.requestId || null;
      this.resources = param?.resources || null;
   ...
   */
  async validate(signature: string, provider?: JsonRpcProvider) {
    console.warn(
  ...
  async verify(
    ...
    return new Promise<SiweResponse>((resolve, reject) => {
      ...
      const invalidParams: Array<keyof VerifyParams> = checkInvalidKeys<VerifyParams>(
        params,
        VerifyParamsKeys
      );
      ...
      if (domain && domain !== this.domain) {
        fail({
          success: false,
          data: this,
          error: new SiweError(SiweErrorType.DOMAIN_MISMATCH, domain, this.domain),
        });
      }
  ...
  private validateMessage(...args: any) {
    ...
    if (!isEIP55Address(this.address)) {
      throw new SiweError(SiweErrorType.INVALID_ADDRESS, getAddress(this.address), this.address);
    }
    ...
    const nonce: any = this?.nonce?.match(/[a-zA-Z0-9]{8,}/);
