...
export const getToken = async (tokenAddress: string, chainId: string) => {
  if (!tokenAddress || !chainId) throw new BadRequestError('tokenAddress and chainId are required');
  ...
  const { tokenData, marketCapInUSD, basePrice, totalFTVolume, totalFTVolumeUsd, pool } =
    fetchedData;
...
export const getTokensBySymbol = async (symbol: string, chainId: string) => {
  ...
  if (!tokens || tokens.length === 0) throw new NotFoundError('Tokens not found');
