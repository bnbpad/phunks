...
export const listTokensController = async (
  ...
  const allTokensDetail = await getTokensDetail(
    chainId,
    skip,
    pageNumber,
    limitNumber,
  ...
  if (sortBy === "marketCap") {
    finalData.sort(
      (a, b) => sortDirection * ((a.usdMarketCap || 0) - (b.usdMarketCap || 0))
    );
  } else if (sortBy === "createdAt") {
    finalData.sort(
      (a, b) => sortDirection * ((a.createdAt || 0) - (b.createdAt || 0))
    );
    ...
    finalData.sort(
      (a, b) =>
        sortDirection * ((a.totalFTVolumeUsd || 0) - (b.totalFTVolumeUsd || 0))
    );
  }
...
export const getTokensDetail = async (
  ...
  const subgraphTokenAddresses = new Set(
    subgraphTokens.map((token) => token.id.toLowerCase())
  );
  ...
  const bnbpadTokensNotInSubgraph = bnbpadTokens.filter(
    (token) =>
      !subgraphTokenAddresses.has(token.basicDetails.address.toLowerCase())
  );
  ...
  const allTokens = [
    ...bnbpadTokenDetails,
    ...bnbpadFallbackDetails,
    ...fourmemeTokenDetails,
  ];
